...
export default function WatchlistPage() {
  ...
  const [items, setItems] = useState<{ itemId: string; itemType: string; title?: string; price?: string; reservePrice?: string }[]>([]);
              ...
              <ul className="divide-y divide-white/5">
                {items.map((w) => (
                  <li key={w.itemId} className="flex items-center justify-between p-3 hover:bg-white/5">
                    <Link href={`/listing/${encodeURIComponent(w.itemId)}`} className="text-white hover:text-chrome font-medium truncate">
                      {w.title || w.itemId}
                    </Link>
