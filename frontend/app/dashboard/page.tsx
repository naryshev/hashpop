    ...
    Promise.all([
      ...
      fetch(`${getApiUrl()}/api/wishlist?address=${encodeURIComponent(address)}`)
        ...
        .then((data: { items?: { itemId: string; itemType: string }[] }) => {
          ...
          ).then((rows) => { if (!cancelled) setWishlistItems(rows); });
        })
        .catch(() => { if (!cancelled) setWishlistItems([]); }),
      fetch(`${getApiUrl()}/api/user/${address}/purchases`)
        .then((res) => (res.ok ? res.json() : Promise.reject(res)))
        .then((data: { purchases?: any[] }) => { if (!cancelled) setPurchaseCount((data.purchases ?? []).length); })
        .catch(() => { if (!cancelled) setPurchaseCount(0); }),
