...
type InboxConversation = {
  ...
  lastMessage: { fromAddress: string; toAddress: string; body: string; createdAt: string; encrypted?: boolean };
  preview: string;
};
...
function MessagesPageContent() {
  ...
  }, [address, composeAddress, composeBody, composeSending, ensureKeypair, fetchPublicKey, fetchInbox]);
                        ...
                        <li key={`${c.otherAddress}-${listingKey}`}>
                          ...
                          >
                            ...
                            <p className="text-silver text-xs mt-1 truncate">
                              {c.lastMessage.encrypted && <LockIcon className="inline-block mr-1 opacity-60" />}
                              {c.preview}
                            </p>
                            <p className="text-silver text-xs mt-0.5">
                          ...
                          <div key={m.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${isMe ? "bg-chrome/25 text-white" : "bg-white/10 text-silver"}`}>
                              <p className="text-xs opacity-80 mb-0.5">
                                ...
                                {m.encrypted && <LockIcon className="inline-block ml-1.5 opacity-60" />}
                              </p>
