 ...
 */
export function getTransactionErrorMessage(error: unknown, options?: { chainId?: number }): string {
  if (error == null) return "";
  const msg = typeof error === "string" ? error : error instanceof Error ? error.message : "";
  const cause =
    error instanceof Error && error.cause != null
      ? typeof (error.cause as Error).message === "string"
  ...
  const dataReason =
    ...
    typeof (o.data as { reason?: string }).reason === "string"
  ...
  const clean = (s: string) => (onHedera ? s.replace(/\b(\d+(?:\.\d+)?)\s*ETH\b/gi, "$1 HBAR") : s);

  if (combined && !/^(error!?|transaction failed|rejected|user denied)$/i.test(combined)) {
    if (
      /transactiontimeouterror|receipt timeout|timed out waiting for transaction receipt/i.test(
        combined,
      )
    ) {
    ...
    if (
      /must have been frozen before calculating the hash|try calling [`'"]?freeze/i.test(combined)
    ) {
      return clean(
